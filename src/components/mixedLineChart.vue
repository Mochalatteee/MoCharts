<template>
  ...
  export default {
    props: {
      ...
      data: {
        type: Array,
        default: () => [20400, 20700, 17400, 9600, 6900, 5900, 6400],
      },
      dataLabel: {
        type: Array,
    ...
    setup(props) {
      ...
      const stackedData = props.dataStacked;
      const stackedDataLabel = props.dataLabelStacked;
  ...
  function drawChart(
    ...
    const spaceWidth = (0.8 * ctnWidth) / stackedData.length;
    const barWidth = 0.6 * spaceWidth;
    const barSpacing = 0.4 * spaceWidth;

    const points = dataToPoints(data, maxData, ctnWidth, ctnHeight,barWidth,barSpacing);
    const fontSize = calculateFontSize(ctnWidth, ctnHeight);
    const stroke = Math.round(Math.min(ctnHeight, ctnWidth) * 0.004);


    const stackedColors = ["#3CA5FA", "#30C9C9", "#FBC645", "#DE558A"];
  ...
  function drawAxis(
    ...
    dataLabel.forEach((value, index) => {
      ...
      let x = 0.15 * ctnWidth + index * (barWidth + barSpacing);
      const text = new Konva.Text({
        x: x,
        y: 0.92 * ctnHeight,
        fontSize: fontSize * 1.2,
  ...
  function dataToPoints(data, maxData, ctnWidth, ctnHeight, barWidth, barSpacing) {
    let points = [];
    const width = 0.8 * ctnWidth;
    const height = 0.8 * ctnHeight;
  
    for (let i = 0; i < data.length; i++) {
        let x = 0.15 * ctnWidth + i * (barWidth + barSpacing) + 0.5*barWidth;
    //   let x = (i * width) / (data.length - 1) + 0.125 * ctnWidth;
      let y = 0.9 * ctnHeight - (data[i] / maxData) * height;
      points.push({ x: x, y: y });
    }
    points.unshift({ x: 0.15 * ctnWidth + 0.5*barWidth, y: 0.9 * ctnHeight });
    points.push({ x: 0.15 * ctnWidth + (data.length - 1) * (barWidth + barSpacing) + 0.5 * barWidth, y: 0.9 * ctnHeight });
...
function drawBars(
  ...
  data.forEach((array, index) => {
    ...
    const text = new Konva.Text({
          text: dataLabel.item[index] + "  Total: " + total,
          fontSize: fontSize * 1.2,
          padding: fontSize,
          fill: "#454545",
          lineHeight: 1.2,
