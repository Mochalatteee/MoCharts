...
export default {
  ...
  setup(props) {
    ...
    onMounted(() => {
      ...
      let data = [];
      if (props.data.length === props.dataLabel.length) {
        for (let i = 0; i < props.data.length; i++) {
          data.push({ value: props.data[i], label: props.dataLabel[i] });
        }
      }

      drawChart(layer, data, ctnWidth, ctnHeight, colors);
    });
...
function drawChart(layer, data, ctnWidth, ctnHeight, colors) {
  ...
  for (let i = 0; i < numSides; i++) {
    let x = center.x + radius * Math.cos((i + 1) * angle - Math.PI / 2);
    let y = center.y + radius * Math.sin((i + 1) * angle - Math.PI / 2)
    const line = new Konva.Line({
      points: [
        center.x,
        center.y,
    ...
    const text = new Konva.Text({
        x: x > 0.5 * ctnWidth ? x  + 0.5 * fontSize: x - data[i].label.length * 0.7 * fontSize,
        y: y > 0.5 * ctnHeight ? y : y - fontSize,
        text: data[i].label,
        align: x > 0.5 * ctnWidth ? 'left': 'right',
        // width: 0.1 * Math.min(ctnHeight,ctnWidth),
        fontSize: fontSize,
        fill:  "#86909C",
    })
